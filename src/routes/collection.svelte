<script lang="ts">
  ...
  import { /* formatBytes, */ mdRegex } from "../lib/utils";

  const query = new URLSearchParams(location.hash?.split("?")[1])
  const collection = getCollection(decodeURIComponent(query.get('name') || ""))
...
{#if collection}
  {#await getCollectionFiles(collection)}
    <div>loading...</div>
  {:then files} 
    <section class="md:grid grid-cols-3 grid-rows-1 h-full">
      <div class="col-span-2 overflow-y-scroll h-full">
        <div class="m-2 flex items-baseline gap-2">
          <h1 class="font-bold text-lg">{collection.name}</h1>
      ...
      {#if $selectedFile}
        <details bind:open={$dialogOpen}>
          ...
          <div class="absolute top-0 left-0 p-4 h-full w-full bg-white md:relative overflow-y-scroll">
