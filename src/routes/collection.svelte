<script lang="ts">
  ...
  async function load() {
    const collection = ref ? getCollection(ref.name) : undefined
    return collection
    ? {
...
<Header />
<Breadcrumbs items={[{ name: "Collections" }, { name: ref?.name || "undefined" }]} />
    ...
    {#if data}
      ...
      <ul class="file-grid">
        {#each data.files as file}
          <li>
            <a href="#/file?ref={Base64.urlEncode({ collection: ref?.name, filename: file.name })}">
              <svg 
